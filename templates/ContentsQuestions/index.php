...
<div class="contents-questions-index">
	...
	if($is_admin_record)
	{
		$course_url = array('controller' => 'contents', 'action' => 'record', $record->course->id, $record->user_id);
	}
	else
	{
		$course_url = array('controller' => 'contents', 'action' => 'index', $content->course->id);
		$this->Breadcrumbs->add(__('コース一覧'), array('controller' => 'users_courses', 'action' => 'index'));
	}
	
	$this->Breadcrumbs->add($content->course->title, $course_url);
	$this->Breadcrumbs->add(h($content->title)); // addCrumb 内でエスケープされない為、別途エスケープ
